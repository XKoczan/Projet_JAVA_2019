...
public class DAO_Eleve extends DAO<Eleve> {
    ...
    public void creer(Eleve obj) {
        try {
            ...
            PreparedStatement stmt = conn.prepareStatement("insert into tab_personne(personne_id,personne_nom,personne_prenom,personne_type) values(?,?,?,?)");
            stmt.setObject(1,id,Types.VARCHAR);
            stmt.setObject(2, nom, Types.VARCHAR);
            stmt.setObject(3, prenom, Types.VARCHAR);
            stmt.setObject(4, type, Types.VARCHAR);
            stmt.executeUpdate();
